...
############################################### 输入要导入数据文件的名称 ###############################################
path_name = os.getcwd()
file_name = 'Auto_run.py'
excel_name = 'Features_table_scaler.xlsx'  # 导入的excel名称
...
for i in range(0, N):
    ...
    mkpath0 = "{}/not-good-splite-data".format(current_path)
    ...
    src_file_list3 = glob.glob(src_dir_info + '*.py')
    ...
    os.rename("{}/train_data.xlsx".format(mkpath), "train_data_for_feature_selection.xlsx")
    shutil.move("{}/train_set/train_data_for_feature_selection.xlsx".format(mkpath), save_path)
    mycopyfile(trian_data_path, "{}/".format(mkpath))
    ...
    time.sleep(1)
    print('****************************The {}th split data!***********************'.format(count))
    count += 1

    os.chdir('{}'.format(mkpath))
    os.system('python Featrue_sele_r_method-loocv.py')
    os.system('python Tensorflow_BPANN.py')
